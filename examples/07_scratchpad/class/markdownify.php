...
class Markdownify {
     ...
     * @var array<string>
     */
    protected static $isMarkdownable = array(
        'p' => array(),
        'ul' => array(),
        'ol' => array(),
        'li' => array(),
     ...
     * @var array<string>
     */
    protected static $wrappableIndents = array(
        '\*   ', # ul
        '\d.  ', # ol
        '\d\d. ', # ol
        '> ', # blockquote
     ...
     */
    protected static $escapeInText = array(
        '([-*_])([ ]{0,2}\1){2,}' => '\\\\$0|', # hr
        '\*\*([^*\s]+)\*\*' => '\*\*$1\*\*', # strong
        '\*([^*\s]+)\*' => '\*$1\*', # em
    ...
    function parse() {
        $this->output = '';
        # drop tags
        $this->parser->html = preg_replace('#<('.implode('|', self::$drop).')[^>]*>.*</\\1>#sU', '', $this->parser->html);
    ...
    function isMarkdownable() {
        if (!isset(self::$isMarkdownable[$this->parser->tagName])) {
        ...
        if ($this->parser->isStartTag) {
            $return = true;
            if ($this->keepHTML) {
                $diff = array_diff(array_keys($this->parser->tagAttributes), array_keys(self::$isMarkdownable[$this->parser->tagName]));
                if (!empty($diff)) {
            ...
            if ($return) {
                foreach (self::$isMarkdownable[$this->parser->tagName] as $attr => $type) {
    ...
    function handleText() {
        ...
        if (!$this->hasParent('code') && !$this->hasParent('pre') && $this->parser->nodeType != 'plain') {
            # entity decode
            $this->decode(&$this->parser->node);
            if (!$this->skipConversion) {
                # escape some chars in normal Text
                $this->parser->node = preg_replace(self::$escapeInText['search'], self::$escapeInText['replace'], $this->parser->node);
            }
        } else {
